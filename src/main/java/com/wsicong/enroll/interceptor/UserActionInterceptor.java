package com.wsicong.enroll.interceptor;
...
public class UserActionInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object obj) throws Exception {
        ...
        if (user != null && StringUtils.isNotEmpty(user.getMobile()) && null != user.getVersion()) {
            // 2. 获取数据库中的用户数据
            User dataUser = this.userService.findUserByMobile(user.getMobile());
            // 3. 对比session中用户的version和数据库中的是否一致
            if (dataUser != null && null != dataUser.getVersion() && String.valueOf(user.getVersion()).equals(
    ...
    public static void out(HttpServletResponse response, ResponseResult result) {
        ...
        try {
            ...
            logger.error("用户在线数量限制【wsicong-enroll-->UserActionInterceptor.out】响应json信息成功");
        } catch (Exception e) {
            logger.error("用户在线数量限制【wsicong-enroll-->UserActionInterceptor.out】响应json信息出错", e);
        } finally {
            if (null != out) {
                out.flush();
                out.close();
