...
import com.wsicong.enroll.dto.UserRoleDTO;
...
import com.wsicong.enroll.mapper.UserRoleMapper;
...
import com.wsicong.enroll.shiro.ShiroRealm;
import com.wsicong.enroll.util.DateUtil;
import com.wsicong.enroll.util.PageDataResult;
import com.wsicong.enroll.util.SendMsgServer;
...
public class UserServiceImpl implements UserService {
    ...
    public String setUser(User user, String roleIds) {
        ...
        if (user.getId() != null) {
            ...
            // 版本不一致
            if (null != dataUser && null != dataUser.getVersion() && !String.valueOf(user.getVersion()).equals(
            ...
            // 方案二【推荐】：用户信息价格flag（或version）标记，写个拦截器，每次请求判断flag（或version）是否改动，
            // 如有改动，请重新登录或自动更新用户信息（推荐）；
    ...
    public String setJobUser(Integer id, Integer isJob, Integer insertUid, Integer version) {
        ...
        if (null != dataUser && null != dataUser.getVersion() && !String.valueOf(version).equals(
                String.valueOf(dataUser.getVersion()))) {
            return "操作失败，请您稍后再试";
        }
